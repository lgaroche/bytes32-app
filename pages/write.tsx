...
const Write: NextPage = () => {
  ...
  const [relayerSigner, setRelayerSigner] = useState<string>(
    "0x9Fd8431229552cEe771E07D5D562D11b71C21d49"
  )
  ...
  const heads = useRef<[unknown, unknown]>()
  ...
  const { data: headReads } = useContractReads({
    ...
    onSuccess: async (data) => {
      if (data && data !== heads.current) {
        const array = ethers.utils.arrayify(data[0] as string)
    ...
    <Page>
      ...
      {progress > 0 && (
        ...
        >
          ...
          >
            <Spinner />
            {progress > 0 && <Text>Waiting for signature...</Text>}
            {progress > 1 && <Text>Publishing...</Text>}
            {progress > 2 && <Text>Waiting for commit...</Text>}
          </Box>
        </Layer>
      )}
    </Page>
